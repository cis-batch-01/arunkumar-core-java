...
public class TopicsDAOImpl implements TopicsDAO {
	...
	static String getTopics = "select id,name,Topics,description from Topics where id=?";
	static String getallTopics = "select id,name,Topics,description from Topics";
	static String insertTopics = "insert into Topics (name,Topics,description)values(?,?,?)";
	static String Updatetopics = "update Topics set name=?,Topics=?,description=? where id=? ";
	...
	public List<Topics> getallTopics() {
		...
		try {
			...
			ResultSet rs = pstmt.executeQuery();
			while (rs.next()) {
				restopics1.add(new Topics(rs.getInt(1), rs.getString(2), rs.getString(3), rs.getString(4)));
