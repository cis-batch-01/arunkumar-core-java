...
select * from transaction_details;
 use bank;
select *,count(transaction_type) as no_of_trans from transaction_details group by transaction_type;
...
on account number.
*/
select account_master.account_number,sum(transaction_details.transaction_amount+account_master.opening_balance) 
as Deposit_Amount  from   transaction_details  
join account_master 
...
where transaction_details.transaction_type="deposit" 
 ...
 /*8.Write a query to display the number of accounts
opened in each city .The Query should display Branch City and number of accounts as
...
select * from account_master;
select * from transaction_details;/**/
/*9..Write a query to display the firstname of the
customers who have more than 1 account. Display the records in sorted order based on
firstname.*/

select customer_master.firstname from customer_master 
join account_master
 on customer_master.customer_number= account_master.customer_number
group by account_master.customer_number having  count(account_master.customer_number)>1
 order by customer_master.firstname;
/*10.Write a query to display the customer number,
customer firstname, customer lastname who has taken loan from more than 1 branch.Display
the records sorted in order based on customer number.*/
select * from customer_master;
select * from branch_master;
select * from loan_details;
select customer_master.CUSTOMER_NUMBER,customer_master.firstname,customer_master.lastname from customer_master 
join loan_details
 on customer_master.customer_number= loan_details.customer_number
group by loan_details.customer_number having  count(loan_details.branch_id)>1
 order by customer_master.firstname;
 /*11.Write a query to display the customer’s number,
customer’s firstname, customer’s city and branch city where the city of the customer and city
of the branch is different. Display the records sorted in ascending order based on customer
number.*/
select * from customer_master;
select * from branch_master;
select * from transaction_details;
select customer_master.CUSTOMER_NUMBER,customer_master.firstname,customer_master.CUSTOMER_CITY,
branch_master.branch_city
from customer_master 
join branch_master
 on customer_master.CUSTOMER_CITY= branch_master.branch_city 
 order by customer_master.firstname;/**/
/*12.Write a query to display the number of clients who
have asked for loans but they don’t have any account in the bank though they are registered
customers. Give the count an alias name of Count.*/
select * from customer_master;
select * from account_master;
select * from loan_details ;
select * from account_master join loan_details
 on loan_details.customer_number=account_master.customer_number;/**/
 /*17.Display the customer number, customer's first name,
account number and number of transactions being made by the customers from each account.
Give the alias name for number of transactions as Count_Trans. Display the records sorted in
...
join account_master
 ...
 on account_master.account_number=transaction_details.account_number
 group by transaction_details.account_number having count(transaction_details.account_number)>1;/**/
/*18.Write a query to display the customer’s firstname
who have multiple accounts (atleast 2 accounts). Display the records sorted in ascending order
based on customer's firstname*/
