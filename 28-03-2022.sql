...
use bank;
CREATE TABLE customer_master( CUSTOMER_NUMBER VARCHAR(6), FIRSTNAME VARCHAR(30), middlename VARCHAR(30), 
lastname VARCHAR(30), CUSTOMER_CITY VARCHAR(15), CUSTOMER_CONTACT_NO VARCHAR(10), occupation VARCHAR(10), CUSTOMER_DATE_OF_BIRTH DATE,
CONSTRAINT customer_custid_pk PRIMARY KEY (CUSTOMER_NUMBER) );
CREATE
TABLE branch_master( branch_id VARCHAR(6), branch_name VARCHAR(30), branch_city VARCHAR(30), CONSTRAINT branch_bid_pk 
PRIMARY KEY (branch_id));
 CREATE TABLE account_master (account_number VARCHAR(255), customer_number VARCHAR(255),branch_id VARCHAR(255), 
 opening_balance INT(20), account_opening_date DATE,account_type VARCHAR(10), account_status VARCHAR(10), PRIMARY KEY
(account_number), FOREIGN KEY (customer_number) references customer_master(customer_number), FOREIGN KEY (branch_id) references
...
select * from account_master;
select * from customer_master;
select account_master.account_number,account_master.customer_number,customer_master.firstname,customer_master.lastname,account_master.account_opening_date 
from account_master join customer_master on customer_master.customer_number=account_master.customer_number order by 
account_master.account_number   asc;
/*2.Please follow instructions given below.Write a query to display the number of customer’s 
from Delhi. Give the count an alias name of Cust_Coun*/
select  count(customer_number)as count from customer_master  group by  customer_city having customer_city='delhi'; 
...
select * from account_master;
select * from customer_master;
select account_master.customer_number,account_master.account_number,customer_master.firstname,account_master.account_opening_date  from account_master 
join customer_master on customer_master.customer_number=account_master.customer_number
where day( account_opening_date)  >15  order by customer_master.customer_number,account_master.account_number asc;
...
select * from account_master;
select * from customer_master;
select account_master.customer_number,account_master.account_number,customer_master.firstname,account_master.account_opening_date,account_master.account_status  from account_master 
join customer_master on customer_master.customer_number=account_master.customer_number where account_master.account_status ="terminated" order by customer_master.customer_number,account_master.account_number asc ;
/*25.Please follow instructions given below.Write a query to display the customers firstname, city and account number whose occupation 
...
select * from customer_master;
select * from account_master;
select  customer_master.firstname,customer_master.customer_city,account_master.account_number from account_master 
join customer_master on customer_master.customer_number=account_master.customer_number where customer_master.occupation  not in( 'bussiness','service','student');
/*24.Please follow instructions given below.Write a query to display the customer number , firstname, customer’s date of birth . Display 
the records sorted in ascending order of date of birth year and within that sort by firstname in 
ascending order.*/
select * from customer_master;
select * from account_master;
select customer_master.customer_number,customer_master.firstname,customer_master.customer_Date_of_Birth from account_master 
join customer_master on customer_master.customer_number=account_master.customer_number
 order by customer_master.customer_Date_of_Birth,customer_master.firstname asc;
 /*23.Please follow instructions given below.Write a query to display customer’s number, first name and middle name. For the customers 
who don’t have middle name, display their last name as middle name. Give the alias name as Middle_Name. */
select customer_number,firstname, coalesce( middlename,lastname)as middle_name from customer_master;
select * from customer_master;
/*22.Please follow instructions given below.Write a query to display account id, customer’s firstname, customer’s lastname for the 
customer’s whose account is Active.Display the records sorted in ascending order based on  account id /account number.*/
select * from customer_master;
select * from account_master;
select account_master.account_number,customer_master.firstname,customer_master.lastname, account_master.account_status from account_master 
join customer_master on customer_master.customer_number=account_master.customer_number where account_master.account_status='active' order by account_master.account_number asc;
...
select *,count(branch_id)as count_branch from branch_master group by  branch_city order by branch_city;
/*20.Please follow instructions given below.Write a query to display the customer’s number, 
...
select * from account_master ;
select * from branch_master;
select customer_master.customer_number,customer_master.firstname,loan_details.branch_id from customer_master 
join loan_details on loan_details.customer_number=customer_master.customer_number order by customer_master.customer_number,loan_details.branch_id;
...
select ,count(branch_id),sum(loan_amount) from loan_details group by customer_number having count(branch_id)>=2;
