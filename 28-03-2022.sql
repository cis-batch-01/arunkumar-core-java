...
select customer_master.customer_Number,customer_master.firstname,customer_master.lastname,sum(loan_amount)as Total_Loan_Amount
from loan_details join customer_master on customer_master.customer_number=loan_details.customer_number
group by loan_details.customer_number having count(branch_id)>=2 order by Total_Loan_Amount desc;
...
select *,count(transaction_type) as no_of_trans from transaction_details;
